...
#     S1 = - (np.sqrt(np.pi) / L) * vp ** 2 * np.exp(- vp ** 2) * np.cos(np.pi * it * DT / 4) * np.sin(2 * np.pi * xp / L) / 4
#     S2 = - (2 * np.sqrt(np.pi) * vp ** 3 / L ** 2) * np.exp(- vp ** 2) * np.sin(np.pi * it * DT / 4) * np.cos(
#         2 * np.pi * xp / L)
#     S3 = 4 * (QM * E / L) * (1 - 0.5 * np.sin(np.pi * it * DT / 4) * np.sin(2 * np.pi * xp / L)) * (np.exp(- vp ** 2) * (vp - vp ** 3)/ np.sqrt(np.pi))
#     return S1 + S2 + S3
...
for h in [1/2, 3/4, 1, 4/3, 2]:
    L = 2 # Length of the container
    DT = .02 * h # Length of a time step
    NT = int(8 / (0.02 * h))  # number of time steps
    NG = int(64 / h) * 2 # Number of Grid points, needs to be even
    ...
    for it in range(NT):
        ...
        mat = sparse.csr_matrix((fraz[0], (p, g[0]))) + sparse.csr_matrix((fraz[1], (p, g[1]))) + sparse.csr_matrix((fraz[2], (p, g[2])))  # interpolation
